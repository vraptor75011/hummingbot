...
class LiquidityMiningStrategyVariant(StrategyPyBase):
...
#        rd = random.uniform(0.5, 1.5)
#        if self._last_submit_timestamp == 0 or (timestamp - self._last_submit_timestamp) > (self._order_refresh_time * rd):
#            self._max_order_age = random.uniform(1, 1.5)
        self.execute_orders_proposal(proposals)
#            self._last_submit_timestamp = timestamp
#            clock_timestamp = pd.Timestamp(timestamp, unit="s", tz="UTC")
#            msg = f"Orders submitted with {self._max_order_age} second {str(clock_timestamp)}"
