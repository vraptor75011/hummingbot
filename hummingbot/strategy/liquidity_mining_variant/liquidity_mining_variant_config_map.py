...
def token_validate(value: str) -> Optional[str]:
    value = value.upper()
    markets = list(liquidity_mining_variant_config_map["markets"].value.split(","))
    tokens = set()
...
def order_size_prompt() -> str:
    token = liquidity_mining_variant_config_map["token"].value
    return f"What is the size of each order (in {token} amount)? >>> "


liquidity_mining_variant_config_map = {
    "strategy": ConfigVar(
        key="strategy",
        prompt="",
        default="liquidity_mining_variant"),
    "exchange":
